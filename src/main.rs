...
pub static MAP: LazyLock<Mutex<HashMap<FileType, mlua::Function>>> = LazyLock::new(|| {
    ...
    let def_file: Function = lua
        .load("function(name, path, tick) return '󰈔 ' .. name end")
        .eval()
        .unwrap();
    let def_dir: Function = lua
        .load("function(name, path, tick) return ' ' .. name .. '/' end")
        .eval()
        .unwrap();
    let def_link: Function = lua
        .load("function(name, path, tick) return ' ' .. name .. '@' end")
        .eval()
        .unwrap();
